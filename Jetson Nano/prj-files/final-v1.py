...
def show_camera():
    ...
    if cap.isOpened():
        ...
        while cv2.getWindowProperty("CSI Camera", 0) >= 0:
            ...
            ctrs = []
            if tot <= 1:
                if (int(time.time())-last_det_ts) > reset_delay:
                    cntr_start = 0
                    crnt_tm = 0
                    tm_df = 0
                if debug:
                    print(f'Cond-1\t tm_df: {tm_df}\tlast_det_tm_df: {int(time.time())-last_det_ts} \tdist: 0')
            ...
            if tot > 0:
                ...
                for i in pos_combs:
                    ...
                    if (tot > 1) and (debug == False):
                        print(f'{tot} persons detected\tDistance: {dist}')
                    # print(f'dist: {dist}')
                    if dist <= distance_threshold:
                      if cntr_start == 0:
                        ...
                        if debug:
                            print(f'Cond-2\t tm_df: {tm_df}\t last_det_tm_df: {last_det_tm_df} \tdist: {dist}')
                      ...
                      cntr_start = int(time.time())
                      if debug:
                        print(f'cond-3\t tm_df: {tm_df}\tdist: {dist}')
                    #   tm_df = crnt_tm - cntr_start
